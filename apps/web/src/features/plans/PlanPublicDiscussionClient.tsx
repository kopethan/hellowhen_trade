  ...
  if (!auth.isAuthenticated) {
    return (
      <PublicDiscussionSignedOutShell
        ...
        heading="Sign in to view the public discussion"
        body="Plan discussion is available to logged-in members so reports and moderation stay accountable."
        actionHref={`/auth?next=${encodeURIComponent(`/plans/${planId}/discussion`)}`}
        actionLabel="Sign in to report"
      />
    );
  }
