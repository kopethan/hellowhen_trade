...
function hideTargetTitle(report: ReportDto) {
  if (report.targetType === 'trade') return 'Trade hidden';
...
function restoreTargetTitle(report: ReportDto) {
  if (report.targetType === 'trade') return 'Trade restored';
...
function actionResultTitle(action: ReportAction, report: ReportDto) {
  if (action === 'hide_target') return hideTargetTitle(report);
  if (action === 'restore_target') return restoreTargetTitle(report);
  if (action === 'suspend_target_owner') return 'Owner suspended';
  if (action === 'unsuspend_target_owner') return 'Owner unsuspended';
...
function actionResultDetails(action: ReportAction, report: ReportDto, supportTicketId?: string) {
  ...
  if (action === 'hide_target') {
    if (report.targetType === 'trade') details.push('This trade is no longer public in discovery/feed surfaces.');
  ...
  if (action === 'restore_target') {
    if (report.targetType === 'trade') details.push('This trade can appear publicly again when otherwise eligible.');
...
export default function AdminReportsPage() {
  ...
  async function applyAction(action: ReportAction) {
    ...
    if ((action === 'hide_target' || action === 'restore_target') && !targetSupportsVisibilityAction(selectedReport)) {
      setNotice({ tone: 'warning', body: targetUnavailableMessage(selectedReport) });
      return;
    }
    setLoading(true);
    setNotice(null);
    try {
      ...
      const title = actionResultTitle(action, data.report);
      setLastAction({ tone: 'success', title, body: `Saved. ${supportLink}`.trim(), details });
      setNotice({ tone: 'success', body: `${title}. ${details.join(' ')}` });
    } catch (error) {
    ...
    <main className="admin-console">
      <section className="app-card admin-console__hero">
        ...
        <div>
          ...
          <h1>Report queue</h1>
          <p>Review reports, batch simple status changes, escalate complex cases to support, and use reversible moderation actions with clear audit notes.</p>
        </div>
        ...
        <article className="app-card admin-action-card">
          {selectedReport ? (
            <>
              ...
              <div className="admin-action-helper">
                <p className="notice-box info"><strong>Actions are reversible.</strong> Use Hide/Restore for supported targets, Suspend/Unsuspend for owner accounts, and Reopen for report review. Every final action keeps audit history and needs an internal note.</p>
                {!targetSupportsVisibilityAction(selectedReport) ? <p className="notice-box warning">{targetUnavailableMessage(selectedReport)}</p> : null}
              </div>
            </>
          ) : (
            <>
