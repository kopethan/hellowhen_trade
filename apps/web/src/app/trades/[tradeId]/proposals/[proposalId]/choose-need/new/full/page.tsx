...
type NewProposalEditNeedFullPageProps = {
...
function chooseNeedHref(tradeId: string, proposalId: string, params: { proposalNeedId?: string; proposalOfferId?: string }) {
  ...
  return `/trades/${tradeId}/proposals/${proposalId}/choose-need/new${query ? `?${query}` : ''}`;
}

export default async function NewProposalEditNeedFullPage({ params, searchParams }: NewProposalEditNeedFullPageProps) {
