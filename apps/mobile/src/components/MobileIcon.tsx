...
function IconPaths({ name, color }: { name: MobileIconName; color: string }) {
  ...
  switch (name) {
    ...
    case 'compass':
      return <><Circle cx={12} cy={12} r={8.25} stroke={color} strokeWidth={1.7} fill="none" /><Path d="M15.75 8.25L13.35 13.35L8.25 15.75L10.65 10.65L15.75 8.25Z" strokeWidth={1.7} {...strokeProps} /><Circle cx={12} cy={12} r={1.05} fill={color} /></>;
    case 'search':
    ...
    case 'share':
      if (Platform.OS === 'ios') {
        return (
          <>
            <Rect x={5} y={9} width={14} height={11.5} rx={2.25} stroke={color} strokeWidth={1.8} fill="none" />
            <Path d="M12 15V3.5" strokeWidth={1.8} {...strokeProps} />
            <Path d="M8.5 7L12 3.5L15.5 7" strokeWidth={1.8} {...strokeProps} />
          </>
        );
      }
