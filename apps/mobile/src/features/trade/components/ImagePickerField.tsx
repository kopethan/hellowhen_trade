          ...
          {images.map((image, index) => {
            ...
            return (
              <View key={image.uri} style={styles.preview}>
                ...
                {enableOrderControls ? <AppText style={styles.coverLabel}>{isCover ? t('inventory.form.coverImage') : t('inventory.form.imageNumber', { number: index + 1 })}</AppText> : null}
                {enableOrderControls ? (
                  <View style={styles.orderRow}>
                    <Pressable accessibilityRole="button" accessibilityLabel={t('inventory.form.moveImageEarlier')} accessibilityState={{ disabled: Boolean(disabled || index === 0) }} onPress={() => moveImage(image.uri, 'up')} disabled={disabled || index === 0} style={({ pressed }) => [styles.orderButton, (disabled || index === 0) && styles.disabled, pressed && styles.pressed]}><AppText style={styles.orderText}>↑</AppText></Pressable>
                    <Pressable accessibilityRole="button" accessibilityLabel={t('inventory.form.moveImageLater')} accessibilityState={{ disabled: Boolean(disabled || index === images.length - 1) }} onPress={() => moveImage(image.uri, 'down')} disabled={disabled || index === images.length - 1} style={({ pressed }) => [styles.orderButton, (disabled || index === images.length - 1) && styles.disabled, pressed && styles.pressed]}><AppText style={styles.orderText}>↓</AppText></Pressable>
                    <Pressable accessibilityRole="button" accessibilityLabel={t('inventory.form.setAsCover')} accessibilityState={{ disabled: Boolean(disabled || isCover) }} onPress={() => setCoverImage(image.uri)} disabled={disabled || isCover} style={({ pressed }) => [styles.orderButtonWide, (disabled || isCover) && styles.disabled, pressed && styles.pressed]}><AppText style={styles.orderText}>{t('inventory.form.coverImage')}</AppText></Pressable>
                  </View>
                ) : null}
