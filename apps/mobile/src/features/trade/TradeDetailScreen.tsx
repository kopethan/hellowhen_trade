...
import { PROPOSAL_MESSAGE_MAX_LENGTH, type MediaAssetDto, type PublicMediaAccessDto, type TradeActionStatus, type TradePostType, type TradeStatus } from '@hellowhen/contracts';
...
function needMeta(need: NeedItem | null | undefined, t: TFunction, trade?: TradeDeckItem) { if (trade && cashPromiseSide(trade) === 'need') return compactList([cashPromiseAmountLabel(trade), t('trade.cashPromise.notProcessed')]); return compactList([need?.category, needTimingBadge(need, t), modeLabel(need?.mode, t), need?.locationLabel]) || t('trade.detail.needDetailsFallback'); }
function offerMeta(offer: OfferItem | null | undefined, t: TFunction, trade?: TradeDeckItem) { if (trade && cashPromiseSide(trade) === 'offer') return compactList([cashPromiseAmountLabel(trade), t('trade.cashPromise.notProcessed')]); return compactList([offer?.includes?.[0], offerTimingBadge(offer, t), modeLabel(offer?.mode, t), offer?.locationLabel]) || t('trade.detail.offerDetailsFallback'); }
...
function hiddenMediaCount(mediaAccess: PublicMediaAccessDto | null | undefined) { return mediaAccess?.requiresAuth && (mediaAccess.hiddenCount ?? 0) > 0 ? mediaAccess.hiddenCount : 0; }
...
export function TradeDetailScreen({ route, navigation }: Props) {
  ...
  const loadTrade = useCallback(async () => {
    ...
    } finally { loadingTradeRef.current = false; setHasLoadedTrade(true); setLoading(false); }
  ...
  const header = <AppHeader title={t('trade.labels.trade')} onBack={() => navigation.goBack()} rightSlot={
    <View style={styles.headerActions}>
      <SavedToggleButton itemType="trade" itemId={trade.id} showLabel={false} hidden={role === 'owner' || !hasLoadedTrade} style={styles.headerShareButton} />
      <Pressable
  ...
  if (!hasLoadedTrade) {
    return <AppFixedHeaderScreen header={header}>
      <TradeDetailLoadingState title={t('trade.detail.loadingTitle')} theme={theme} />
    ...
    <>
      ...
      >
        ...
        <View style={styles.heroActionRow}>
          <AddToAgendaButton sourceType="trade" sourceId={trade.id} itemType="trade" title={detailTitle(trade, t)} note={trade.description} style={styles.heroAgendaButton} />
        </View>
    ...
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive">
      <View style={styles.loadingBlock}>
        ...
        <View style={styles.loadingGroup}>
          <View style={[styles.loadingLine, styles.loadingLineWide, { backgroundColor: theme.color.subtleSurface }]} />
          <View style={[styles.loadingLine, styles.loadingLineMedium, { backgroundColor: theme.color.subtleSurface }]} />
          <View style={[styles.loadingLine, styles.loadingLineShort, { backgroundColor: theme.color.subtleSurface }]} />
        </View>
        <View style={[styles.loadingSection, { borderTopColor: theme.color.border }]}>
          <View style={[styles.loadingLine, styles.loadingLineShort, { backgroundColor: theme.color.subtleSurface }]} />
          <View style={[styles.loadingLine, styles.loadingLineWide, { backgroundColor: theme.color.subtleSurface }]} />
          <View style={[styles.loadingLine, styles.loadingLineMedium, { backgroundColor: theme.color.subtleSurface }]} />
        </View>
...
function ProposalComposer({ trade, requiredSide, value, onChange, onSubmit, loading, sideLoading, needs, offers, selectedNeedId, selectedOfferId, onChooseNeed, onChooseOffer, theme, t }: { trade: TradeDeckItem; requiredSide: RequiredProposalSide; value: string; onChange: (value: string) => void; onSubmit: () => void; loading: boolean; sideLoading: boolean; needs: NeedItem[]; offers: OfferItem[]; selectedNeedId: string; selectedOfferId: string; onChooseNeed: () => void; onChooseOffer: () => void; theme: ThemeTokens; t: TFunction }) {
  ...
  return (
    <View style={styles.composerBox}>
      ...
      <InfoNotice tone="info" title={t('trade.privateProposalsEntry.optionalDetailsTitle')} body={t('trade.privateProposalsEntry.optionalDetailsBody')} />
...
function ThreadActionCard({ title, body, meta, icon, tone, onPress, theme, t }: { title: string; body: string; meta: string; icon: 'discussion' | 'proposal'; tone: 'trade' | 'proposal'; onPress: () => void; theme: ThemeTokens; t: TFunction }) {
  return (
