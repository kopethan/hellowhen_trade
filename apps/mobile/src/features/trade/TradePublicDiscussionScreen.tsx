...
export function TradePublicDiscussionScreen({ route, navigation }: Props) {
  ...
  function startReply(message: TradePublicMessageItem) {
    if (message.status === 'deleted') return;
  ...
  const contextTitle = route.params.title || t('trade.publicDiscussion.tradeContext');
    ...
    <AppFixedHeaderScreen header={<AppHeader title={t('trade.publicDiscussion.title')} onBack={() => navigation.goBack()} rightSlot={<ThreadMenuButton label={t('trade.publicDiscussion.threadMenuTitle')} onPress={() => setThreadMenuVisible(true)} />} />}>
      <KeyboardAvoidingView style={styles.shell} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={Platform.OS === 'ios' ? 88 : 0}>
        <PublicThreadContextStrip
          label={t('trade.publicDiscussion.tradeContext')}
          title={contextTitle}
          actionLabel={t('trade.publicDiscussion.viewTrade')}
          theme={theme}
        ...
        <>
          ...
          {!deleted ? (
            <View style={styles.messageActionRow}>
              ...
              <InlineCommentAction label={isOwn ? t('trade.publicDiscussion.messageActions') : t('trade.publicDiscussion.reportMessage')} tone={isOwn ? 'default' : 'danger'} theme={theme} onPress={isOwn ? onOwnActions : onReport} />
            </View>
          ) : null}
        </>
      )}
