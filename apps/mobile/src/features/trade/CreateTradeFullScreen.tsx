      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive" showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={loading} onRefresh={() => { void loadResources(); }} />}>
        ...
        <AppCard>
          ...
          <View style={styles.expiryRow}>{expiryOptions.map((option) => {
            ...
            return <Pressable key={option.labelKey} disabled={submitting} onPress={() => setExpiryDays(option.days)} style={({ pressed }) => [styles.expiryButton, { backgroundColor: theme.color.surface, borderColor: theme.color.border }, selected && { backgroundColor: theme.semantic.trade.softBg, borderColor: theme.semantic.trade.border }, submitting && styles.disabled, pressed && styles.pressed]}><AppText style={[styles.expiryButtonText, { color: selected ? theme.semantic.trade.text : theme.color.muted }]}>{t(option.labelKey)}</AppText></Pressable>;
          })}</View>
        ...
        <View style={styles.actions}>
          <Pressable disabled={submitting || loading} onPress={handlePublish} style={({ pressed }) => [styles.primaryButton, { backgroundColor: theme.semantic.trade.bg }, (submitting || loading) && styles.disabled, pressed && styles.pressed]}><AppText style={[styles.primaryButtonText, { color: theme.semantic.trade.onBg }]}>{submitting ? t('trade.create.publishing') : publishButtonLabel}</AppText></Pressable>
