...
export function MyOffersScreen() {
    ...
    <AppHeader
      ...
      rightSlot={(
        <LibraryHeaderActions
          tone="offer"
          state={controls}
          searchAccessibilityLabel={t('inventory.libraryFilters.searchMyOffers')}
          filterAccessibilityLabel={t('inventory.libraryFilters.filterMyOffers')}
  ...
  return (
    <AppSmartHeaderScreen header={header} resetKey="offer">
      {(scrollProps) => (
        <ScrollView {...scrollProps.scrollViewProps} contentContainerStyle={[scrollProps.contentInsetStyle, styles.content]} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'} refreshControl={<RefreshControl refreshing={loading} onRefresh={() => { void loadItems(); setFolderRefreshKey((key) => key + 1); }} />}>
          <AppText style={[styles.subtitle, { color: theme.color.muted }]}>{t('inventory.empty.offerNativeBody')}</AppText>
