export type ExploreDiscoveryKind = 'trade' | 'plan' | 'need' | 'offer' | 'place';

export const EXPLORE_DISCOVERY_KIND_ORDER: readonly ExploreDiscoveryKind[] = ['trade', 'place', 'need', 'plan', 'offer'];

export function createExploreDiscoverySeed() {
  return (Date.now() ^ Math.floor(Math.random() * 0xffffffff)) >>> 0;
}

export function hashExploreDiscoveryValue(value: string) {
  let hash = 2166136261;
...
export function buildBalancedExploreDiscoveryFeed<T extends { kind: ExploreDiscoveryKind; key: string }>(
  groups: Record<ExploreDiscoveryKind, readonly T[]>,
  seed: number,
) {
  ...
  ])) as Record<ExploreDiscoveryKind, T[]>;

  const mixed: T[] = [];
  let cycle = 0;
  let previousKind: ExploreDiscoveryKind | null = null;
