...
TIERS11 decision document plus TIERS12 shared constants/types.

This document is for future implementation planning. It does not enable checkout, billing, Stripe, Airwallex, Apple in-app purchases, Google Play Billing, RevenueCat, entitlement mutation, Pro activation, or Business onboarding.
...
No live native in-app purchases
No billing provider SDKs in the app
...
Apple/Google product ids should be documented in the app-store setup checklist later.

## Environment variable direction later

Do not add these in TIERS11.

Likely future web billing variables:
...
Likely future mobile billing flags:

```env
EXPO_PUBLIC_IOS_SUBSCRIPTIONS_ENABLED=false
EXPO_PUBLIC_ANDROID_SUBSCRIPTIONS_ENABLED=false
```
